# Database Schema Specification — KPA EMC² Web Portal

> **Engine:** PostgreSQL 18+ · **Laravel Migrations** · **Eloquent ORM**
>
> **Standar Arsitektur:**
>
> 1. **Total 12 Tabel (Clean & Modular):** Taksonomi dipusatkan di `categories` dan array tags `string[]` di `posts` dan `events`.
> 2. **Constraint Array / JSONB:** Kolom `tags` divalidasi dengan `CHECK (jsonb_typeof(tags) = 'array')` dan diindeks dengan GIN Index.
> 3. **Relasi Divisi Terpadu:** Seluruh entitas konten & kegiatan (`posts`, `events`, `galleries`, `members`) memiliki foreign key opsional `division_id` (NULLABLE).
> 4. **5 Kolom Audit Log Wajib:** `is_active`, `created_by`, `updated_by`, `created_at`, `updated_at` di setiap tabel.
> 5. **Zero-BLOB Policy:** Seluruh media disimpan di Cloudinary (`*_url` & `*_public_id`).
>    (Lihat detail aturan: [`db_rules.md`](file:///home/ali-musthafa-kamal/projects/kpa-emc2-web/docs/database/db_rules.md)).

---

## Table List

| #   | Table Name      | Description                                                           | Scope         | Division Related?  | Category Related?  |    Tags Array     | New? |
| --- | --------------- | --------------------------------------------------------------------- | ------------- | :----------------: | :----------------: | :---------------: | ---- |
| 1   | `users`         | Admin & editorial accounts (Filament panel)                           | Core Auth     |         —          |         —          |         —         | new  |
| 2   | `about_infos`   | Organization profile, vision, mission, history                        | Profile       |         —          |         —          |         —         | new  |
| 3   | `divisions`     | Operational divisions of KPA EMC² (Kaderisasi, SKLH, Litbang, Karata) | Profile       |       Parent       |         —          |         —         | new  |
| 4   | `members`       | Member roster with division, position, and batch                      | Profile       | ✅ (`division_id`) |         —          |         —         | new  |
| 5   | `categories`    | Unified taxonomy for posts, events, galleries                         | Taxonomy      |         —          |       Parent       |         —         | new  |
| 6   | `posts`         | Articles, expedition journals, nature notes                           | Content       | ✅ (`division_id`) | ✅ (`category_id`) | ✅ (`tags` array) | new  |
| 7   | `galleries`     | Photo/video album collections                                         | Media         | ✅ (`division_id`) | ✅ (`category_id`) |         —         | new  |
| 8   | `gallery_items` | Individual media items within albums                                  | Media         |         —          |         —          |         —         | new  |
| 9   | `events`        | Organized activities open for public registration                     | Events        | ✅ (`division_id`) | ✅ (`category_id`) | ✅ (`tags` array) | new  |
| 10  | `registrations` | General participant registrations for any event                       | Events        |         —          |         —          |         —         | new  |
| 11  | `contacts`      | Inbound messages from public contact form                             | Communication |         —          |         —          |         —         | new  |
| 12  | `site_settings` | Global website config as key-value pairs                              | System        |         —          |         —          |         —         | new  |
| 13  | ...             | continue if adding new table                                          | ...           |         —          |         —          |         —         | new  |
...
| `slug`              | `VARCHAR(100)`                          | NOT NULL, UNIQUE                  | e.g. `mountain-forest`, `caving`, `rock-climbing` |
...
| `tags`                  | `JSONB`                               | NOT NULL, DEFAULT `'[]'::jsonb`, CHECK (`jsonb_typeof(tags) = 'array'`) | Tags array `string[]`: `["survival", "navigasi"]` |
...
- `UNIQUE(slug)`
- `CHECK(jsonb_typeof(tags) = 'array')` (Database-level array constraint)
- `INDEX(category_id)`
- `INDEX(division_id)`
- `INDEX(user_id)`
...
## Modern PostgreSQL Architecture & Scalability Highlights

### 1. Partial Indexing for High Read Performance

Public feed queries only hit rows that are published and active, skipping drafts and soft-deleted records:
...
### 2. Typo-Tolerant Search & Full-Text Search

Using PostgreSQL `pg_trgm` and `tsvector` without needing external Elasticsearch/Meilisearch:

```sql
CREATE EXTENSION IF NOT EXISTS "pg_trgm";
CREATE EXTENSION IF NOT EXISTS "unaccent";

-- Substring & fuzzy search on titles
CREATE INDEX idx_posts_title_trgm ON posts USING GIN (title gin_trgm_ops);

-- Indonesian full-text search on article content
CREATE INDEX idx_posts_content_fts ON posts USING GIN (to_tsvector('indonesian', title || ' ' || excerpt || ' ' || content));
```

### 3. Native `JSONB` for Dynamic Forms & Tag Arrays

Tag filtering and custom form values queryable with GIN indexes:

```sql
-- Tag filtering on posts & events:
CREATE INDEX idx_posts_tags ON posts USING GIN (tags);
CREATE INDEX idx_events_tags ON events USING GIN (tags);

-- Query posts with tag 'survival':
SELECT * FROM posts WHERE tags @> '["survival"]';
```

### 4. Migration Execution Ordering
...
3.  categories
4.  about_infos
5.  members        (FK: divisions, users)
6.  posts          (FK: users, categories, divisions)
7.  galleries      (FK: divisions, categories, users)
8.  gallery_items  (FK: galleries, users)
9.  events         (FK: users, categories, divisions)
10. registrations  (FK: events, users)
11. contacts       (FK: users)
12. site_settings  (FK: users)
```
